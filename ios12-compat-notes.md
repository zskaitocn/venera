...
- 移除 iOS 14+ 专属键：`CADisableMinimumFrameDurationOnPhone`, `UIApplicationSupportsIndirectInputEvents`

### 4. AppDelegate.swift (`ios/Runner/AppDelegate.swift`)
- `import UniformTypeIdentifiers` → `import MobileCoreServices`（iOS 8-14 兼容）
- `UIDocumentPickerViewController(forOpeningContentTypes:...)` → `UIDocumentPickerViewController(documentTypes:...)`（iOS 14 新方法在 iOS 12 不可用）
